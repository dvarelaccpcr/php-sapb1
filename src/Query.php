...
class Query
{
     ...
     */
    public function __construct(Config $config, array $session, $serviceName, array $headers)
    {
        $this->config = $config;
     ...
     */
    public function limit($top, $skip = 0)
    {
        $this->query['top'] = $top;
     ...
     * AND operation.
     */
    public function where(Filters\Filter $filter)
    {
     ...
     * OR operation.
     */
    public function orWhere(Filters\Filter $filter)
    {
     ...
     */
    private function doRequest($action = '', callable $callback = null)
    {
